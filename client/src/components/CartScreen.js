...
export default function CartScreen() {
  ...
  const updateCartHandler = async (item, quantity) => {
    const { data } = await Axios.get(`/Plant/backend/client/src/components/Category.js${item._id}`)
                  ...
                  <ListGroup.Item key={item._id}>
                    <Row className="align-items-center">
                      <Col md={4}>
                        ...
                        <Link to={`/Plant/backend/client/src/components/Categories.js/${item.slug}`}> {item.name} </Link>
                      </Col>
                      <Col md={3}>
